...
int main() {
    ifstream arq("microinstrucoes_etapa3_tarefa1.txt");
    ofstream log("saida_etapa3_tarefa1.txt");
    Mic1 m;
    m.carregarRegistradores("registradores_etapa3_tarefa1.txt");
    m.imprimirEstadoInicial(log);
    string linha;
    int ciclo = 1;
    while (getline(arq, linha)) {
        if (linha.size() == 23) {
            m.executarInstrucao(linha, ciclo++, log);
        }
    }
    log << "=====================================================\n";
    log << "Cycle " << ciclo << "\nNo more lines, EOP.\n";
